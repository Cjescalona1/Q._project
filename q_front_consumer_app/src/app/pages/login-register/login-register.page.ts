...
export class LoginRegisterPage implements OnInit {
 ...
 async onSubmit(mode:boolean) {
    ...
    if(mode){
      await this.authApi.consumerLogin(this.loginForm.value, this.user).then(res=>{ this.user = res });
      console.log(this.user);
      sessionStorage.setItem('token',JSON.stringify(this.user?.token));
      sessionStorage.setItem('user',JSON.stringify(this.user?.Consumer.id));
    }
    else{

      await  this.authApi.consumerReg(this.registerForm.value, this.userR).then(res=>{
      console.log("res",res);
