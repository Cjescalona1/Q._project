...
export class ServicesService {
  ...
  async findConsumer(params : any): Promise<any> { 
    let aux: any ;
    let output :any ; 
    aux = params

    await fetch(URL.api+'/consumers/'+aux, 
      {
        method:'GET',
        headers: {
                'Content-Type': 'application/json',   
  ...
  async editProfile(params: any, req: any):Promise<any> {
    console.log(params);
    
    let aux; 
    fetch(URL.api+'/consumers/'+params.profile_id,{
      method:'PUT',
      body: JSON.stringify(params),
      headers: {
              'Content-Type': 'application/json',   
  ...
  async rateAppointment(appointment:any, params: any, rating:any): Promise<any> {
    let aux: any ;  
    let output :any ; 
    aux = JSON.stringify({status:params, rating:rating})
