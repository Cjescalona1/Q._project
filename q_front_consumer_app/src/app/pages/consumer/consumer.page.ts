...
async handleEventClick(clickInfo: EventClickArg) { 
  ...
  const modal = await this.modalCtrl.create({
    ...
    componentProps: { _aux : aux, _self : false }
  });
  modal.present( );
...
public _Appointment :any;
  async ngOnInit() {
    ...
    //get provider
    await this.servApi.findConsumer(JSON.parse(queryString)).then(res=>{this._cons=res.consumer}); 
    this._cons = this.servApi.b64toCss([this._cons])[0]
    ...
    await this.calService.getConsumerAppointments(url[2]).then(res=>{this._Appointment=res.appointments });     
  ...
  goEdit(id:any){
    window.location.assign("/edit-profile/"+id)
  }
